...
const AudioPlaylist: React.FC = () => {
  const { globalVolume, autoplayTriggered, setAutoplayTriggered, isPlaying, setIsPlaying, trackToPlay, audioOn, setAudioOn } = useAudio();
  ...
  const playTrack = useCallback((idx: number, forceLoop: boolean = false) => {
    ...
    // Only attempt to play audio if it's not muted globally
    if (audioOn) {
      // Play the track with silent error handling using our utility function
      playSilently(audioRef.current);
    }
    setIsPlaying(true); // Update the global playing state even if muted
  }, [cueTrack, tracks, loopTrack, loopedTrackId, setIsPlaying, showError, audioOn]);
  ...
  useEffect(() => {
    if (autoplayTriggered && !isLoading && tracks.length > 0) {
      ...
      const randomIndex = Math.floor(Math.random() * tracks.length);
      playTrack(randomIndex, false); // Play a random track without forcing loop - playTrack already respects audioOn
  ...
  }, [isPlaying, currentTrackIndex, tracks, audioOn]);
    ...
    <>
      <section className="bg-black min-h-[calc(100vh-4rem)] w-full flex flex-col items-center justify-center snap-start p-4 md:p-8">
        ...
        <ul className="space-y-3 max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-fuchsia-600 scrollbar-track-neutral-800 pr-2">
          {tracks.length === 0 ? (
            <li className="p-3 md:p-4 rounded-md bg-neutral-800 text-fuchsia-300 text-center font-medium">
              No Tracks Available
            </li>
          ) : (
      ...
      {isMobile && (
        <button
          className="fixed bottom-4 right-4 z-50 p-4 rounded-full bg-gradient-to-r from-fuchsia-600 to-pink-600 text-white shadow-lg shadow-fuchsia-500/30 hover:shadow-fuchsia-500/50 transition-all"
          onClick={() => {
            if (!audioOn) {
              setAudioOn(true);
            }
            ...
            if (audioRef.current) {
              ...
              // No audio element yet, play first track
              playTrack(0);
            }
          }}
